...
int http_get(int connection, const char *path, const char *host, const char *proxy_host, const char *proxy_user, const char *proxy_password) {
  ...
  if (proxy_user) {
    ...
    sprintf(proxy_credentials, "%s:%s", pproxy_user, proxy_password);
...
int main(int argc, char *argv[]) {
  ...
  proxy_host = proxy_user = proxy_passsword = host = path = NULL;
  ind = 1;
  ...
  http_get(client_connection, path, host, proxy_hostm proxy_user, proxy_password);
